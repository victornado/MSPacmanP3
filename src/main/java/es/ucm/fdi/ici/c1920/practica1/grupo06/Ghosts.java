...
import es.ucm.fdi.ici.c1920.practica1.grupo06.Pair;
...
public class Ghosts extends GhostController {
	...
	public EnumMap<GHOST, MOVE> getMove(Game game, long timeDue) {
		...
		if(isSomeGhostEdible(game)) {
		...
		else if(PacmanNearestCornerWithPowerPill(game, pacmanIndex, 40) != -1){
			
			defensiveStrategyHalfGhostsRun(game, pacmanIndex);
			return moves;
			...
			for (GHOST gh : GHOST.values()) {
				...
				if(isPacmanNearPowPill(game, pacmanIndex, 15)){
	...
	private void defensiveStrategyHalfGhostsRun(Game game, int pacmanIndex) {
		...
		for (int i = 0; i < 2; i++) {
			GHOST aux = nearestGhostTo(game, pacmanIndex, (GHOST[])ghost_list.toArray(ghosts_array), DM.EUCLID);
			ghost_list.remove(aux);
			//if(isPacmanNearPowPill(game, pacmanIndex, 10))
				//moves.put(aux, game.getNextMoveAwayFromTarget(game.getGhostCurrentNodeIndex(aux), pacmanIndex, DM.PATH));
			//else
				moves.put(aux, game.getNextMoveTowardsTarget(game.getGhostCurrentNodeIndex(aux), pacmanIndex, DM.PATH));
			ghosts_array = new GHOST[ghost_list.size()];
		}
		
		for (GHOST gh : ghost_list) {
	...
	*/	
	...
	private void defensiveStrategyDefendPill(Game game, int pacmanIndex) {
		...
		for (GHOST gh : GHOST.values()) {
			if(!gh.equals(GHOST.BLINKY))
				moves.put(gh, moveTowardUnlessPacmanIsNear(game, gh, game.getActivePillsIndices()[game.getActivePillsIndices().length/2], DM.MANHATTAN, pacmanIndex, 5)); 
		}
	}
	...
	static protected Pair<Integer,Double>[] getFourNearestJunctionsTo(Game game, int fromIndex) {
	...
	*/
	...
	Si no se encuentra dentro del limite, devuelve -1
 	@Params
		...
		limit -> distancia limite de pacman a la esquina
 */
	private int PacmanNearestCornerWithPowerPill(Game game, int indexPacman, int limit) {
	...
	*/
	static protected GHOST nearestGhostTo(Game game, int index, GHOST[] ghosts, DM dm) {
		GHOST minGh = null;
