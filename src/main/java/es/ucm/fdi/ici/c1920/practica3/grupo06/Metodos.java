...
public class Metodos {
	...
	private static int[][] mazeIndexes = { { 0, 78, 1191, 1291 }, { 0, 86, 1217, 1317 }, { 0, 78, 1300, 1378 },
			{ 0, 100, 1218, 1307 } };
	private static MOVE[] allMoves = MOVE.values();
	private static Random rnd = new Random();

	/*
	 * Devuelve el index de la esquina más cercana a pacman sin power pill dentro de
	 * un limite. Si no se encuentra dentro del limite, devuelve -1
	 * 
	 * @Params game -> clase game con las funciones. pacmanIndex -> index de pacman
	 ...
	 * 
	 * @Params game -> clase game con las funciones. pacmanIndez -> el index de
	 * pacman
	 */
	...
	/*
	 * Devuelve un array de 4 pares <nodo, distancia>, siendo estos 4 los junctions
	 * más cercanos a pacman
	 * 
	 * @Params game -> clase game con las funciones.
	 */
	public static Pair<Integer, Double>[] getFourNearestJunctionsTo(Game game, int fromIndex) {
		...
		for (int i = 1; i < junctions.length; i++) {
			Pair<Integer, Double> newest = new Pair<Integer, Double>(junctions[i],
					game.getDistance(fromIndex, junctions[i], DM.PATH));
	 ...
	 * 
	 * @Params game -> clase game con las funciones. index -> index al que se quiere
	 * comprobar la distancia. ghosts -> fantasmas que se quieren comprobar. Si el
	 * array es null, se comprueban todos los fantasmas dm -> tipo de distancia
	 */
	...
	/*
	 * Devuelve el movimiento hacia un index de un fantasma, a no ser que pacman
	 * este dentro de un radio limit. En ese caso, devuelve el movimiento hacia
	 * pacman.
	 * 
	 * @Params game -> clase game con las funciones. gh -> fantasma del que se
	 * quiere calcular el movimiento toindex -> index al que se quiere mover. dm ->
	 * tipo de distancia pacmanIndex -> index de pacman. limit -> limite del
	 * fantasma hasta pacman.
	 */
	public static MOVE moveTowardUnlessPacmanIsNear(Game game, GHOST gh, int toIndex, DM dm, int pacmanIndex,
			int limit) {
	...
	public static GHOST getNearestChasingGhost(Game game, double limit, int pacmanIndex) {
		...
		for (GHOST gh : GHOST.values()) {
			int node = game.getGhostCurrentNodeIndex(gh);
			double dist = -1;
			if (node != -1) {
				dist = game.getDistance(pacmanIndex, node, DM.MANHATTAN);
	...
	public static boolean GhostCanBlockMe(Game game, int pacmanIndex, MOVE moveMade) {
		...
		for (GHOST gh : GHOST.values()) {
			if (game.getDistance(game.getGhostCurrentNodeIndex(gh), juncDest, DM.PATH) < pacmanDist
					&& game.getGhostLastMoveMade(gh) != game.getPacmanLastMoveMade())
				return true;
		}
